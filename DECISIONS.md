...
- **Choix** : option C.
- **Vérification par exécution (exigée par DEC-0036 avant toute montée de cran)** : mesure en lecture seule sur le VPS, sous les deux utilisateurs concernés. (1) **Le contrôle proposé est infaisable là où il devrait tourner** : `check-project.sh` s'exécute depuis le dossier projet, donc sous `jihad`, et `ls /root/.hermes/skills/` renvoie `Permission denied` ; les projets vivent sous `/home/jihad/projects/`, les skills d'Hermès sous `/root/.hermes/`. Le contrôle resterait silencieux exactement là où on l'attend. (2) **La dérive redoutée n'existe pas** : une seule skill de projet est réellement installée chez Hermès, `blue-app` ; `diff -rq` avec sa source `MySecretaire` ne relève **aucun écart**, malgré 24 jours d'écart de date de modification, versions déclarées identiques (1.2.0). (3) **L'écart réel est ailleurs et il est correct** : 8 des 9 skills de projet ne sont pas installées chez Hermès, dont les 7 de Mediatros, ce qui est précisément la conclusion de l'audit de portabilité de ce projet ; un contrôle de couverture ne produirait donc que des faux positifs. (4) **La vraie dérive est dans le périmètre de T-RETEX-2 mais hors du dépôt** : entre le profil `mysecretaire` et le dossier global d'Hermès, **15 skills sur 31 divergent**, sur 15 profils portant chacun 31 à 36 skills. Ce constat est une mesure, pas un diagnostic : cette divergence peut être une personnalisation voulue par Hermès, dont MyProjectOS ne connaît pas la sémantique.
...
- **Liens** : CHG-20260807-2230, `RETEX/retex-mediatros-retrofit-skills-portables.md`, `RETEX/retex-alldebrid-faux-diagnostic-cles-bws.md`, T-RETEX-1, T-RETEX-2. Applique DEC-0036 (montée de cran adossée à une preuve exécutée). Complète DEC-0037 (partage `platforms:` / `portable:`). Prolonge DEC-0034 (skills portables, installation par agent).
...
- **Options envisagées** :
  ...
  - C. Corriger les affirmations **et** porter les filtres natifs au squelette, en articulant le champ `portable:` du RETEX Mediatros avec `platforms:` au lieu de le doubler.
  - D. Idem C, plus un contrôle de taille des `SKILL.md` dans `check-project.sh` sur un seuil recalculé.
- **Choix** : option C.
- **Raison** : A propage une règle inventée dans toute skill créée depuis le squelette, et déclencherait un refactoring de deux fichiers qui sont en réalité à 40 % et 54 % du budget officiel Anthropic (500 lignes de corps). D est réfutée par DEC-0036 elle-même, qui interdit de transformer une supposition en règle du système et exige une preuve exécutée avant toute montée de cran : outiller un seuil dont on vient d'établir qu'il ne s'applique pas serait le contre-exemple parfait de la règle posée la veille. B laisse sur la table un mécanisme natif, gratuit et rétrocompatible qui traite la cause même du RETEX Mediatros (huit skills macOS proposées puis échouant sur un VPS Linux, alors que `platforms: [macos]` les aurait écartées avant l'offre). C remplace de la documentation par un mécanisme, ce qui est le sens même de l'échelle de fermeté de DEC-0036.
