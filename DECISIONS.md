...
- **Choix** : C, avec adoption. `skills.external_dirs` devient la voie canonique côté Hermès ; la copie physique globale est abandonnée ; une variante propre à un agent passe par un lien symbolique dans le dossier de skills du profil. L'enseignement d'instrument est consigné comme règle dans `docs/skills-portables.md`. A est écarté : DEC-0036 interdit de transformer une réponse de modèle en règle du système, et le RETEX service tiers de téléchargement portait déjà sur un faux diagnostic remonté par une skill. B est écarté parce qu'une preuve mal instrumentée n'invalide pas la thèse, elle oblige à la mesurer correctement. D est écarté parce qu'il outillait la surveillance d'un artefact qui, sur un déploiement profilé, n'était même pas offert à l'agent.
- **Raison** : la déclaration supprime le second exemplaire au lieu d'en surveiller la dérive, ce qui referme définitivement le sujet ouvert par le constat 5 du RETEX Projet Gamma et par T-RETEX-2. Elle a aussi un mérite que la copie n'avait pas : elle porte sur le dossier, donc toute skill future du catalogue arrive sans geste d'installation. Et le défaut de la copie globale était plus grave que la dérive qu'on lui reprochait, puisqu'elle pouvait exister sur le disque sans jamais être offerte.
...
### DEC-0038 — RETEX Projet Gamma arbitré : cinq évolutions retenues, contrôle de dérive Hermès écarté sur preuve

- **Date** : 2026-08-07
- **Contexte** : le RETEX Projet Gamma (le RETEX correspondant, CHG-20260807-1915) proposait six évolutions, laissées à l'arbitrage sous T-RETEX-1. La sixième, un contrôle de dérive entre la copie Hermès d'une skill et sa source projet, portait la même règle que la seule piste canonique du RETEX service tiers de téléchargement (T-RETEX-2), les deux devant être instruites ensemble. Le RETEX laissait par ailleurs un point explicitement ouvert : pour `mainwp-ssh`, `platforms: [macos]` produit le bon effet mais pour une raison techniquement fausse.
- **Options envisagées** :
...
- **Choix** : option C.
- **Vérification par exécution (exigée par DEC-0036 avant toute montée de cran)** : mesure en lecture seule sur le VPS, sous les deux utilisateurs concernés. (1) **Le contrôle proposé est infaisable là où il devrait tourner** : `check-project.sh` s'exécute depuis le dossier projet, donc sous `l'utilisateur`, et `ls [chemin serveur]` renvoie `Permission denied` ; les projets vivent sous `[chemin serveur]`, les skills d'Hermès sous `[chemin serveur]`. Le contrôle resterait silencieux exactement là où on l'attend. (2) **La dérive redoutée n'existe pas** : une seule skill de projet est réellement installée chez Hermès, `blue-app` ; `diff -rq` avec sa source `Projet Beta` ne relève **aucun écart**, malgré 24 jours d'écart de date de modification, versions déclarées identiques (1.2.0). (3) **L'écart réel est ailleurs et il est correct** : 8 des 9 skills de projet ne sont pas installées chez Hermès, dont les 7 de Projet Gamma, ce qui est précisément la conclusion de l'audit de portabilité de ce projet ; un contrôle de couverture ne produirait donc que des faux positifs. (4) **La vraie dérive est dans le périmètre de T-RETEX-2 mais hors du dépôt** : entre le profil `Projet Beta` et le dossier global d'Hermès, **15 skills sur 31 divergent**, sur 15 profils portant chacun 31 à 36 skills. Ce constat est une mesure, pas un diagnostic : cette divergence peut être une personnalisation voulue par Hermès, dont MyProjectOS ne connaît pas la sémantique.
...
- **Liens** : CHG-20260807-2230, le RETEX correspondant, le RETEX correspondant, T-RETEX-1, T-RETEX-2. Applique DEC-0036 (montée de cran adossée à une preuve exécutée). Complète DEC-0037 (partage `platforms:` / `portable:`). Prolonge DEC-0034 (skills portables, installation par agent).
...
- **Options envisagées** :
  ...
  - C. Corriger les affirmations **et** porter les filtres natifs au squelette, en articulant le champ `portable:` du RETEX Projet Gamma avec `platforms:` au lieu de le doubler.
  - D. Idem C, plus un contrôle de taille des `SKILL.md` dans `check-project.sh` sur un seuil recalculé.
- **Choix** : option C.
- **Raison** : A propage une règle inventée dans toute skill créée depuis le squelette, et déclencherait un refactoring de deux fichiers qui sont en réalité à 40 % et 54 % du budget officiel Anthropic (500 lignes de corps). D est réfutée par DEC-0036 elle-même, qui interdit de transformer une supposition en règle du système et exige une preuve exécutée avant toute montée de cran : outiller un seuil dont on vient d'établir qu'il ne s'applique pas serait le contre-exemple parfait de la règle posée la veille. B laisse sur la table un mécanisme natif, gratuit et rétrocompatible qui traite la cause même du RETEX Projet Gamma (huit skills macOS proposées puis échouant sur un VPS Linux, alors que `platforms: [macos]` les aurait écartées avant l'offre). C remplace de la documentation par un mécanisme, ce qui est le sens même de l'échelle de fermeté de DEC-0036.
...
- **Date** : 2026-08-07
- **Contexte** : la méthode capitalise déjà par `RETEX/`, `DECISIONS.md`, les skills et les hooks, mais aucune règle ne dit quand une leçon monte en fermeté, ni qui referme un RETEX. Le plan Pro Workflow identifiait ce manque (A3, un plan interne, priorité « très haute »). La revue complète du backlog d'évolution du 2026-08-07 l'a démontré sur pièces : sur 7 RETEX, 3 portaient un statut faux ou jamais mis à jour (`98_configuration` et `doublon-archives` marqués « à évaluer » alors qu'ils sont intégrés depuis v0.7.0/v0.8.0 ; Projet Delta marqué « à convertir » alors que sa section 1 est intégrée depuis v0.5.0), et 4 plans dormaient sans qu'on sache s'ils avaient été refusés ou simplement oubliés. Le mot « RETEX » n'était même pas défini dans le glossaire, alors que trois documents du canon s'appuient dessus. Aggravant : la veille upstream d'août signale que Harness v5 a ajouté un audit log de déclenchement de règles, c'est-à-dire la traçabilité de l'enforcement, précisément ce qui manquait ici en amont.
- **Options envisagées** :
...
- **Liens** : CHG-20260807-1923, A3 de un plan interne, T-PLAN-6. Prolonge DEC-0016 (une règle sans détection est creuse) et DEC-0033 (un écart assumé ne se re-signale pas indéfiniment).
...
- **Choix** : option C.
- **Raison** : la portabilité multi-agents ne se limite pas aux outils catalogués — dès qu'un projet est piloté par plusieurs agents, n'importe quelle skill bespoke rencontre le même besoin (cas vérifié : les six skills métier bespoke d'un autre projet). Le lien symbolique supprime la dérive source/copie plutôt que de la déplacer (B la laisse identique au risque déjà documenté). Rester optionnel (A) laisserait chaque projet réinventer sa propre variante, comme cet autre projet l'a fait en dehors du canon.
...
- **Date** : 2026-07-12
- **Contexte** : DEC-0027/0028 posent la gouvernance Blue (`GOUVERNANCE_BLUE.md`) et sa proposition active, mais la méthode ne livrait qu'un gabarit documentaire, pas d'outillage technique installable. un plan interne (T-PLAN-4, englobe T-PLAN-2/T-PLAN-3) propose qu'un projet qui active Blue reçoive en plus une skill technique portable (CLI + GraphQL + résolution de secrets), agnostique agent (Claude Code, Codex, Hermès), et un protocole d'équipement inter-agents. Cinq points d'architecture nécessitaient un arbitrage humain (D1-D5, § 10 du plan) ; tranchés par l'utilisateur le 2026-07-12.
- **Options envisagées et choix (D1-D5)** :
  ...
  - **D4 — Migration de `~/.claude/skills/blue-cli`** (myagent, skill globale personnelle antérieure au plan) vers une instance de `blue-app`, débloquée une fois la Phase 2 (banc d'essai Projet Beta) validée en réel.
...
- **Choix** : option C.
- **Raison** : A répète l'erreur déjà actée en DEC-0026 (convention non canonisée = redécouverte à chaque projet). B aurait documenté un mode observé uniquement hors méthode (Projet Zeta n'a pas de `TASKS.md`), sans valeur pour un projet piloté par MyProjectOS, au prix d'un gabarit plus long — écarté après clarification avec l'utilisateur. C ne canonise que ce qui est confirmé par deux sources indépendantes (pièges CLI) ou directement dérivé du fichier sacré `TASKS.md` (workflow de miroir), sans figer prématurément un usage à observation unique (principe déjà tenu en DEC-0002).
...
- **Liens** : CHG-20260712-1110, le RETEX correspondant.
...
- **Contexte** : `init-project.sh` câblait `.claude/settings.json` de chaque projet avec un chemin absolu vers `MyProjectOS/scripts/hooks/*.sh`. Les gabarits Markdown étaient copiés (isolés), mais les hooks référencés à distance : un projet dépendait donc du dépôt `MyProjectOS` restant en place à ce chemin exact, et modifier un script de hook affectait silencieusement tous les projets déjà créés (4 projets concernés au moment du constat, aucun averti). Repéré lors d'un audit du projet Projet Alpha, qui a demandé l'arrêt de ce partage.
- **Options envisagées** :
...
- **Conséquences** : chaque nouveau projet reçoit sa propre copie des hooks, modifiable indépendamment des autres. Corriger un bug de hook dans `MyProjectOS` ne se propage plus automatiquement : recopie projet par projet, ou `--update-method` depuis la v0.5.0. Les 4 projets créés avant cette décision (Projet Alpha, HomeLab, Assistant_Juridique, Projet Epsilon) gardent l'ancien câblage tant qu'ils ne sont pas migrés individuellement.
...
### DEC-0024 — SUJETS.md à la racine, source fraîche prioritaire (intégration du RETEX Projet Delta)

- **Date** : 2026-07-07
- **Contexte** : le RETEX Projet Delta (2026-06-14) montre que les demandes métier (« dépenses maison ») ne sont pas bien routées : `docs/INDEX.md` décrit la carte documentaire mais pas le vocabulaire utilisateur, et une synthèse peut être moins fraîche que le fichier source.
...
- **Liens** : CHG-20260613-2055, CHG-20260613-2226. Détail du raisonnement : un plan interne.
