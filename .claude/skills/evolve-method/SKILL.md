...
- **RETEX** : retour d'expérience terrain (dogfood Life+Knowledge) au format « problème → correction locale → évolution générique à intégrer ». Ses sections « Évolution générique attendue » sont un backlog non traité. Tout nouveau RETEX suit ce format.
...
La méthode n'a jamais été validée sur un vrai projet Code : banc d'essai Projet Zeta (phase 6 de TASKS.md), bloqué derrière la validation du plan Company OS. Le passage à `1.0.0` dépend de ce cap (DEC-0015). [read: from PROGRESS.md, DECISIONS.md]
