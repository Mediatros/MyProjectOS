...
prochaine_action: T-R.3 (installeur non aveugle), propagation v0.10.0 aux trois projets structurés (Projet Alpha en premier), banc d'essai Projet Zeta (Phase 6), ou T-B.7 / T-C.11. Signalé : Projet Epsilon a 10 bloquants préexistants (fichiers sacrés Hybrid jamais greffés, --into-existing à envisager).
prochaine_echeance:
---
...
- Un document de passation interne : base de travail initiale, non figée. Tout peut être remis en question.
...
- Plans : des plans internes de construction et d'analyse/décisions, `PLAN/` (plans et documents de travail isolés avant intégration).
- Inspiration : `Projet Zeta` (naming, index, format PROGRESS, identifiants CHG).
...
- v0.9.0 posée le 2026-07-12 : idée « Blue en brique optionnelle » traitée (DEC-0027, CHG-20260712-1130). Comparaison de deux projets Blue indépendants (Projet Alpha/`myagent`, mode miroir `TASKS.md` ; Projet Zeta/`Projet Zeta`, hors méthode, registre autonome) a confirmé au niveau outil des pièges CLI communs (`-w` obligatoire sur update, description multi-ligne → `Unterminated string`, checklist en deux commandes) et, au niveau usage, que seul le mode miroir `TASKS.md` est pertinent pour un gabarit MyProjectOS (Projet Zeta n'a pas de `TASKS.md`). Nouveau gabarit `templates/configuration/GOUVERNANCE_BLUE.md`, pré-rempli (nommage, checklists, workflow de synchro, pièges CLI), à préférer au gabarit générique vide `GOUVERNANCE_INTEGRATION.md`. `docs/NAMING-CONVENTIONS.md` et la skill assistant signalent la préséance d'une variante pré-remplie par outil. **Reste à faire** : propagation `--update-method` aux trois projets structurés (Projet Alpha, Projet Delta, Projet Epsilon) ; le `GOUVERNANCE_BLUE.md` existant de Projet Alpha n'a pas besoin d'être réaligné sur le gabarit (il reste la source vécue, plus riche sur ses spécificités locales) — pas encore fait.
- v0.8.0 posée le 2026-07-12 (Phase S, T-S.1 à T-S.5) à partir de l'issue GitHub #1 / RETEX Projet Alpha `98_configuration/` : dossier optionnel canonisé pour la gouvernance d'intégrations tierces partagées entre agents et le handoff asynchrone inter-agents (gabarits génériques `templates/configuration/HANDOFF_INTERAGENT.md` et `GOUVERNANCE_INTEGRATION.md`), garde-fou temps réel étendu aux collisions de préfixe `NN_` dans `hook-pre-write.sh` (referme l'angle mort des abréviations que T-R.1/T-R.2 ne couvraient pas), consigne active dans la skill qui clôt au passage T-R.4. Voir DEC-0026, CHG-20260712-1110. **Reste à faire** : `sh scripts/validate.sh`-équivalent (skill `validate`) pour vérifier par exécution (projet test, hook testé sur une collision de préfixe), puis propagation `--update-method` aux trois projets structurés (Projet Alpha en premier, à l'origine du RETEX) — pas encore fait.
- v0.7.0 publiée le 2026-07-10 (tag + release GitHub, CI verte) puis propagée via `--update-method` aux trois projets structurés (Projet Alpha, Projet Delta, Projet Epsilon, tous 0.6.0 → 0.7.0, migrations consignées dans leur CHANGELOG). Apport : garde-fous dossiers racine (T-R.1/T-R.2). Le nouveau contrôle passe à [ok] sur les trois projets.
...
- RETEX Projet Alpha du 2026-07-09 (le RETEX correspondant, CHG-20260709-2350) : un quasi-doublon de dossier racine (`99_archives`/`99_archive`) a traversé les trois couches d'enforcement, qui contrôlaient les fichiers mais pas les dossiers. Phase R : T-R.1, T-R.2 (2026-07-09) et T-R.4 (2026-07-12, généralisé via Phase S) faits ; reste T-R.3 (installeur). Propagé aux projets avec la v0.7.0.
