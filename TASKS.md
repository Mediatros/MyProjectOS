...
- [x] **T-PLAN-8** Extension Knowledge v2 — **arbitré le 2026-08-22 (DEC-0043, CHG-20260822-0918)** : structure 4 niveaux rejetée après challenge multi-modèles (deepseek-pro, ox-alpha, kimi-k3 ; sol indisponible, quota Codex) ; doctrine « carte jamais territoire » intégrée (3 niveaux + zone froide unique `99_archive/knowledge/`). Modifications appliquées : `templates/extensions/knowledge/docs/kb_governance.md` réécrit, `structures/knowledge-tree.md` complété, `scripts/check-project.sh` §8 étendu (budget carte, liens SUJETS.md, .bak hors archive), `AGENTS.md` rituel carte (write bloqué par protection fichiers agent, à poser), plan marqué « arbitré — fusion partielle ».
...
- [ ] **T-PLAN-6** Arbitrer les améliorations restantes du plan Pro Workflow (un plan interne). **A3 (boucle de correction gouvernée) retenue et faite le 2026-08-07** (DEC-0036, CHG-20260807-1923, v0.18.0). Restent, dans l'ordre convenu : A6 (inventaire lisible des hooks), puis A1 (garde-fou Git destructif) et A2 (scan local de secrets), une version chacun, chacun avec sa matrice de test. A4 (clôture déterministe Code) après A6. **A5 (index FTS5 Knowledge) est sorti de ce plan** : il apparaît aussi dans T-PLAN-7 (option B) et T-PLAN-8, et doit faire l'objet d'un arbitrage groupé unique sous peine de produire trois moteurs. Renfort de la veille d'août : Harness v5 a ajouté R15 (blocage du staging de secrets) et un audit log de déclenchement de règles, qui recoupent A2 et A6.
