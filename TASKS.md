...
- [ ] **T-RETEX-1** Arbitrer les six évolutions du RETEX Projet Gamma (le RETEX correspondant, CHG-20260807-1915) : mode « rattrapage » d'un parc de skills existant (artefact `98_configuration/AUDIT_SKILLS.md`), champ `portable:` à quatre valeurs au frontmatter du squelette, expression du cas « non portable par décision » (section « ne pas installer » dans l'`INSTALL.md`), tableau d'inventaire dans le `README.md` de `98_configuration/skills/`, renvoi vers `templates/skills/_squelette/` dès qu'un projet outille sa portabilité ou ses secrets, et contrôle de dérive de la copie Hermès dans `check-project.sh` (constat 5, ajouté le 2026-08-07). **Évolution 2 reconfigurée par DEC-0037** : `portable:` ne double plus `platforms:`, il porte ce que la machine ne peut pas déduire (`partiel`, `conditionnel`, « non par décision »), et toute valeur `non`/`partiel` d'origine technique doit s'accompagner d'un `platforms:` restrictif. Point resté à trancher : pour `mainwp-ssh`, `platforms: [macos]` donne le bon effet mais pour une raison techniquement fausse. Aucune modification du squelette avant décision, hors celles de T-HERMES-1.
- [ ] **T-RETEX-2** Statuer sur la seule piste du RETEX service tiers de téléchargement touchant le canon (le RETEX correspondant, CHG-20260802-2341) : contrôle d'empreinte anti-dérive des skills répliquées, dans `check-project.sh` ou ailleurs. Les deux autres pistes (installation par lien symbolique des skills Hermès par profil, documentation des deux chemins BWS) relèvent de la gouvernance Hermès, hors dépôt méthode. **À instruire avec la sixième évolution de T-RETEX-1** : même règle sur deux périmètres (copie Hermès divergente de sa source projet ; skills Hermès répliquées entre profils d'un même hôte), une seule décision.
