...
- [ ] **T-PLAN-4** Dérouler le plan `PLAN/plans/2026-07-12-blue-brique-complete-skill-agnostique.md` (brique Blue complète, englobe T-PLAN-2 et T-PLAN-3 ; décisions D1-D5 tranchées le 2026-07-12). **Phase 1 faite le 2026-07-12** (scripts + secrets, testés en réel). **Phase 2 faite le 2026-07-12** (banc d'essai MySecretaire : skill validée par exécution sur les trois agents — Claude Code/keychain, Codex/keychain, Hermès VPS/file + couche Bitwarden env —, D3 figé sur la copie globale, `INSTALL.md` propagé aux 4 instances, tableau d'équipement rempli, handoff « Équiper un agent » EN_ATTENTE vers Hermès). Voir CHG-20260712-1345. **Phase 3 faite le 2026-07-12** (recettes GraphQL par domaine exécutées en réel sur MySecretaire et figées dans le `SKILL.md` : documents/wiki, formulaires, discussions, chat, status updates, fichiers avec upload presigned validé de bout en bout via le nouveau `scripts/blue-files.sh` ; table de routage complète ; skill `1.1.0` propagée aux 4 instances ; critères T-PLAN-2 § 4 tous satisfaits ; voir CHG-20260712-1658). **Phase 4 faite le 2026-07-12** (workspace modèle `_Modele_MyProjectOS` créé, converti en template avec accord explicite, copie vérifiée via workspace fils jetable puis supprimé ; incident « inviter l'utilisateur sur un workspace créé par API » résolu et documenté ; workflow commentaires testé en réel sur une carte MySecretaire, mécanisme lecture → traitement → confirmation validé ; nouveaux pièges CLI consignés ; voir CHG-20260712-1720). **Phase 5 câblage interne fait le 2026-07-12** (skill assistant Mode 5/6 propose `blue-app` + noms d'agents canoniques + chemins Hermès + `98_configuration/skills/` + handoff « Équiper un agent » ; DEC-0029, v0.11.0, voir CHG-20260712-1900). Reste : propagation `--update-method` aux projets structurés (LaCIOTAT, Comptabilite_globale, TeamLeader), mise à jour des `GOUVERNANCE_BLUE.md` déjà instanciées (LaCIOTAT, MySecretaire), migration `~/.claude/skills/blue-cli` vers une instance `blue-app` (D4) — session dédiée.
