...
- [x] **T-PLAN-4** Brique Blue complète (un plan interne, décisions D1-D5 tranchées le 2026-07-12) : skill technique portable `blue-app` (scripts + secrets multi-backend), validée sur les trois agents (Phase 2), recettes GraphQL par domaine (Phase 3), workspace modèle et workflow commentaires (Phase 4), câblage méthode + propagation `--update-method` aux projets structurés + migration D4 (`~/.claude/skills/blue-app-myagent/`, `blue-cli/` conservée en filet) (Phase 5). DEC-0029, v0.11.0 publiée le 2026-07-12 (tag + release GitHub). Voir CHG-20260712-1345, 1658, 1720, 1900, 2000. Reste hors décision explicite de l'utilisateur : bascule effective (suppression de `blue-cli/`, conservée en filet).
- [x] **T-ISSUES-1** Traitement des 3 issues GitHub (2026-07-12) : #1 clôturée (canonisation `98_configuration/`, déjà couverte par v0.8.0-v0.11.0) ; #2 (`tags add` écrase les tags, bug CLI Homebrew) mitigée dans le wrapper `blue-cli.sh` (fusion automatique des tags existants, testée en réel) ; #3 (`todoCustomFields` null en GraphQL) résolue par la recette `customFields { id name value }` (résolveur `todoCustomFields` cassé côté serveur Blue). Skill `blue-app` 1.2.0, propagée (instances Projet Beta, `blue-app-myagent`, handoff VPS Hermès). v0.11.1 publiée. Voir CHG-20260712-2145.
- [x] **T-HERMES-1** Appliquer DEC-0037 (v0.19.0). **Fait le 2026-08-07, voir CHG-20260807-2115.** Détail de ce qui était demandé : (a) retirer de `templates/skills/_squelette/SKILL.md` l'affirmation « `SKILL.md` sous 20 000 caractères (seuil de troncature Hermès) », qui ne correspond à aucun mécanisme réel et se recopie dans chaque skill créée depuis le squelette ; (b) documenter le champ natif `platforms:` au frontmatter du squelette (règle « absent égale compatible partout », validé par exécution 6 cas sur 6 le 2026-08-07) et l'articuler avec la section d'installation par agent de `templates/skills/_squelette/INSTALL.md` ; **ne pas** porter `environments:` au canon, écarté après test (faux positif de détection `docker` sur un hôte où Docker est installé, et ses trois valeurs sont internes à l'infra Hermès) ; (c) corriger `agents/hermes.md` (seuil de 20 000 devenu le plancher d'un calcul dynamique dérivé de la fenêtre du modèle, troisième levier = modèle à plus grande fenêtre, documentation des deux filtres natifs). `scripts/check-project.sh` reste inchangé, `HERMES_MAX_CHARS=20000` étant requalifié en seuil de prudence (c'est le repli réel quand la fenêtre est inconnue) et non en limite. Ne **pas** ajouter les `SKILL.md` au contrôle de taille. Preuves : un plan interne.
- [x] **T-UPD-1** Réparer la chaîne de détection des mises à jour de méthode dans les projets. **Fait le 2026-08-08 (DEC-0039, CHG-20260808-0015, v0.21.0).** Défaut diagnostiqué sur `SYNC/Projet Alpha` (projet en v0.11.0, méthode en v0.20.0), qui affichait `[ok] suit MyProjectOS v0.11.0 (version courante)` : la section 0 de `check-project.sh` compare `PROJECT.md` au `VERSION` du projet, deux fichiers rafraîchis ensemble par `--update-method`, donc elle ne peut pas voir l'amont ; et le Mode 7 de la skill attendait un avertissement de ce contrôle, impossible à produire. Corrigé par l'option (a) : la détection passe au rituel de reprise (étape 3 du Mode 1, `check-update.sh` une fois par session, silencieux hors réseau ou si à jour), le Mode 7 pointe désormais vers ce constat, et le contrôle local cesse de parler de « version courante ». Vérifié par exécution sur cinq cas dont le hors-réseau (code 0) et le retard réel (code 10). Option (b) écartée : elle aurait rendu `check-project.sh` dépendant du réseau. Option (d), garantie déterministe par hook réseau, laissée ouverte comme montée de cran distincte.
- [x] **T-RETEX-1** Arbitrer les six évolutions du RETEX Projet Gamma. **Fait le 2026-08-07 (DEC-0038, CHG-20260807-2230, v0.20.0).** Cinq retenues et appliquées : rattrapage du parc en déclencheur du Mode 2 de la skill (pas de huitième mode), champ `portable:` optionnel à quatre valeurs au squelette (documentation assumée, sans contrôle associé), section « ne pas installer » et tableau « limites par environnement » dans l'`INSTALL.md`, gabarit `templates/configuration/README_SKILLS.md` (un seul artefact de parc au lieu des deux proposés), renvoi vers `templates/skills/_squelette/` dès qu'un projet outille sa portabilité ou ses secrets. Point ouvert tranché : une restriction par décision ne se traduit **pas** en `platforms:`, la non-installation est le mécanisme correct. Sixième évolution écartée sur mesure, voir T-RETEX-2.
- [x] **T-RETEX-3** Arbitrer les évolutions du RETEX Projet Beta skills portables Hermès. **Fait le 2026-08-08 (DEC-0040, CHG-20260808-1130, v0.22.0).** Les trois évolutions sont retenues et appliquées : E1 `skills.external_dirs` canonisé comme voie Hermès et copie physique globale abandonnée, E2 sixième étape du Mode 7 offrant le passage du parc de skills en mode portable après une mise à jour, E3 motif « deux versions d'une même skill » documenté. Ajout au passage : OpenCode acté comme quatrième agent (découvre `.claude/skills/` et `.agents/skills/`, rien à installer). **La preuve du RETEX a dû être refaite** : ses mesures avaient été prises avec `hermes skills list`, qui n'observe pas le registre offert à l'agent (224 / 174 / 148 pour trois objets différents sur le même profil) ; re-mesurée sur `scan_skill_commands()`, la thèse tient. Le RETEX est corrigé sur place et passé à `integre`. Nouvelle règle au canon : mesurer avec l'instrument que l'agent utilise. Referme définitivement le constat 5 du RETEX Projet Gamma et T-RETEX-2, la copie surveillée ayant disparu au lieu d'être contrôlée.
- [x] **T-RETEX-2** Contrôle d'empreinte anti-dérive des skills répliquées. **Clos le 2026-08-07 (DEC-0038)**, instruit avec la sixième évolution de T-RETEX-1 comme prévu. Écarté du canon sur preuve exécutée : `check-project.sh` s'exécute sous `l'utilisateur` et ne peut pas lire `[chemin serveur]` (`Permission denied`) ; aucune dérive sur le seul cas comparable (`blue-app` identique à sa source) ; les 8 skills de projet absentes chez Hermès le sont volontairement, donc un contrôle de couverture ne produirait que des faux positifs. La divergence réelle mesurée (15 skills sur 31 entre le profil `Projet Beta` et le dossier global d'Hermès, sur 15 profils) est hors du dépôt méthode et relève de la gouvernance Hermès, comme les deux autres pistes du RETEX service tiers de téléchargement.

## En cours

- [x] **T-PUB-1** Acter la publication de v0.21.0 (tag + release) : **close le 2026-08-22 (CHG-20260822-1736)** — v0.22.0 absorbe v0.21.0, aucun tag rétroactif ; chaîne continue v0.22.0 → v0.23.0. Publication v0.23.0 faite le même jour.
- [ ] **T-PLAN-10** Maintenir ouverte la candidature MyProjectOS de `writing-unslop` (un plan interne) sans préinstallation ni modification du canon. **Conservation locale décidée** : l'utilisateur garde une légère préférence pour cette skill et demande qu'elle reste installée dans Hermes. Deux tests réalisés : léger avantage sur un texte synthétique, aucune différence utile perçue sur un email réel face à `humanizer` et `redaction-humaine-fr`. Poursuivre avec des cas discriminants ; clarifier la licence avant toute redistribution.
- [ ] **T-RETEX-4** Arbitrer le RETEX préliminaire Projet Beta sur la progression hiérarchique multi-sujets, après la fenêtre d’observation du 2026-08-13 au 2026-08-20 : examiner dérives parent/local, conflits Syncthing, discipline local → parent et pertinence de la lecture ciblée du changelog. Source : le RETEX correspondant. Aucun changement du canon avant RETEX final et décision humaine.
- [ ] **T-PLAN-9** Arbitrer Agent Plugins v1 portables comme format de distribution optionnel des capacités MyProjectOS (un plan interne) : figer d'abord le contrat officiel, traiter la mise à jour Hermès comme un chantier séparé, puis exécuter si validé un POC isolé sur la seule skill `my-project-os` et comparer avec `skills.external_dirs`. Décider ensuite rejet / usage ciblé skill+MCP / distribution Hermès / format commun. Aucun changement de méthode, aucun MCP et aucune mise à jour Hermès sans GO distinct. T7.1 est reliée à ce plan et ne doit pas être exécutée séparément.
- [x] **T-PLAN-8** Extension Knowledge v2 — **arbitré le 2026-08-22 (DEC-0043, CHG-20260822-0918)** : structure 4 niveaux rejetée après challenge multi-modèles (deepseek-pro, ox-alpha, kimi-k3 ; sol indisponible, quota Codex) ; doctrine « le sommaire, pas tout le livre » intégrée (3 niveaux + zone froide unique `99_archive/knowledge/`). Modifications appliquées : `templates/extensions/knowledge/docs/kb_governance.md` réécrit, `structures/knowledge-tree.md` complété, `scripts/check-project.sh` §8 étendu (budget sommaire, liens SUJETS.md, .bak hors archive), `AGENTS.md` rituel sommaire, plan marqué « arbitré — fusion partielle ».
- [ ] **T-PLAN-7** Arbitrer le plan de mémoire dérivée multi-niveaux (un plan interne) : choisir d’abord le besoin prioritaire (compression, rappel, capitalisation, Knowledge ou Code-Graph), vérifier ce que couvrent déjà Mnemosyne et l’index FTS5 candidat, puis décider entre reprise conceptuelle, prototype natif minimal, POC TencentDB isolé ou rejet. Aucun plugin, moteur mémoire ou changement Hermes avant décision explicite.
- [x] **T-PLAN-6** Améliorations du plan Pro Workflow (un plan interne) — **close le 2026-08-23**. Cinq évolutions retenues, implémentées et testées en réel, une décision chacune :
  - **A3** boucle de correction gouvernée — 2026-08-07, DEC-0036, v0.18.0.
  - **A6** inventaire lisible des hooks — 2026-08-22 : section 12 « Hooks » de `check-project.sh` (câblé sans fichier, fichier non câblé, `_lib.sh` hors périmètre ; avertissements seulement), gate validé sur Projet Alpha (incluant un hook non-méthode) et sur un cas cassé monté pour test.
  - **A1** garde-fou Git destructif — DEC-0044 : `scripts/hooks/hook-pre-git.sh` bloque les 5 irréversibles (`push --force/-f`, `reset --hard`, `clean -fd/-x`, `branch -D`, rebase branche distante), dérogation traçable `MYPROJECTOS_GIT_OVERRIDE=1`, activation optionnelle Code/Hybrid ; matrice 14/14.
  - **A2** scan local de secrets — DEC-0045 : `scripts/check-secrets.sh` transverse, deux niveaux (bloque PEM + préfixes tokens connus, avertit affectations suspectes et fichiers .env/.pem/.key), valeur jamais affichée, exclusions `.myprojectos/secrets-allow`, appelé par la section 13 du check ; matrice 11/11.
  - **A4** clôture déterministe Code — DEC-0046 : `scripts/check-iteration.sh` lancé explicitement en clôture (mode 4 de la skill), deux bloquants (fichiers non consignés dans PROGRESS.md, PROGRESS périmé), câblé à TEST_PLAN.md quand présent sans exigence artificielle pour les projets documentaires ; 4 scénarios testés.
  - **A5** (index FTS5 Knowledge) sortie du plan : arbitrage groupé unique avec T-PLAN-7/T-PLAN-8 sous peine de produire trois moteurs. Renfort noté : Harness v5 a ajouté R15 (blocage staging de secrets) et un audit log de règles, recoupant A2/A6. Voir CHG-20260823-0845 et précédents.
- [ ] **T-PLAN-1** Relire et valider un plan interne avant intégration dans les docs stables.
- [ ] **T-PLAN-5** Catalogue d'outils natifs `98_configuration/` (un plan interne) : catalogue `docs/OUTILS.md`, squelette de skill portable, brique secrets transverse (`secrets.sh` générique, age sur VPS, voie gratuite obligatoire), outil AgentMail (skill maison dérivée de l'officielle), relevé des handoffs au rituel de reprise, skills utilitaires sans compte (`courrier-manuscrit`, `redaction-humaine-fr`), outil Tailscale (vérification + onboarding guidé, préalable d'Infisical VPS). Décisions D1-D6 toutes tranchées le 2026-07-13, formalisées dans DEC-0030. **Phase 1 faite le 2026-07-13** (v0.12.0 : `docs/OUTILS.md`, squelette + `secrets.sh` générique testé 7/7, `courrier-manuscrit` générisée + `redaction-humaine-fr` copiée telle quelle, skill assistant au catalogue). **Phase 2 remaniée le 2026-07-13 (DEC-0031)** : Infisical auto-hébergé abandonné après échec d'installation en réel (3 h sur le VPS) ; VPS = SOPS + age (backend `sops` ajouté à `secrets.sh`, script `ajout-secret.sh`, hook Telegram `/secret` hors LLM documenté dans `agents/hermes.md`), v0.13.0. Restent : Phase 2 en réel (Tailscale, parcours sops, hook `/secret` validé par test à blanc, migration des `.env` du VPS), Phase 3 (AgentMail), Phase 4 (handoff via PROGRESS). Voir CHG-20260713-1120, 1140, 1302, 1330, 1923.
...
### Phase R — RETEX Projet Alpha : intégrité des dossiers racine
But : qu'un quasi-doublon de dossier racine (type `99_archives`/`99_archive`, survenu dans Projet Alpha du 2026-07-07 au 2026-07-09) soit bloqué à la création ou, à défaut, détecté à l'audit. Analyse complète : le RETEX correspondant.
...
### Phase S — RETEX Projet Alpha : `98_configuration/`, gouvernance et handoff inter-agents
But : donner une case canonique aux projets pilotés par plusieurs agents (gouvernance d'intégrations tierces partagées, handoff asynchrone). Analyse complète : le RETEX correspondant.
...
### Phase T — RETEX Projet Beta : bloc d'en-tête PROGRESS.md systématique
But : que le bloc d'en-tête frontmatter de `PROGRESS.md` (`derniere_maj`, `prochaine_action`, `prochaine_echeance`) existe dans tous les projets et soit mis à jour à chaque mise à jour du fichier, sans dérive. Besoin né de la skill radar de Projet Beta, qui lit ces en-têtes pour produire un tour d'horizon inter-projets à bas coût.

- [ ] **T-T.1** Migrer les `PROGRESS.md` des projets nés avant le template : ajouter le bloc d'en-tête frontmatter. Projets SYNC sans bloc au 2026-07-12 : `Automatisation_Magazine`, `Biblioteca-SaaS`, `Copropriete_Dupont`, `Gestion_Courrier`, `Lindet_Litige_Final`, `Projet Zeta` (plus les projets LOCAL non migrés). Passer par le runbook de migration (`docs/versioning.md`) ou `--into-existing`.
...
- [ ] **T6.1** Onboarder Projet Zeta via `init-project.sh` (profil Code).
