...
- `plans/2026-08-07-verification-limites-hermes-skills.md` — **document de preuve, pas un plan** : vérification terrain du code Hermès sur le VPS (commit `015aae836`) établissant que la troncature à 20 000 caractères ne s'applique **pas** aux `SKILL.md` (seulement aux cinq fichiers de contexte racine), que ce seuil est devenu un plancher dynamique dérivé de la fenêtre du modèle, et qu'Hermès filtre déjà nativement les skills par `platforms:`/`environments:` au frontmatter. Contient les numéros de ligne, les contre-preuves empiriques, les limites assumées de la vérification et les commandes exactes de re-vérification. Statut : résultats acquis, appliqués par DEC-0037 (T-HERMES-1) et reconfigurant l'évolution 2 du RETEX Mediatros (T-RETEX-1).
- `plans/2026-08-04-extension-knowledge-v2-4-niveaux.md` — hiérarchie documentaire à 4 niveaux (N1 Macro constitution / N2 Méso thématique / N3 Micro fiche vivante / N4 Archives froid), issue d'une proposition Hermes, à confronter à l'extension Knowledge actuelle (niveaux 1/2/3) : niveau d'archive formel, portée du N1, nommage N3 miroir du N2 parent, budgets de taille, métaphore de température, enforcement déterministe. Statut : document de travail, sept écarts à trancher point par point avant toute décision d'adoption (T-PLAN-8).
- `plans/2026-07-31-agent-memory-integration.md` — plan d’intégration inspiré de TencentDB Agent Memory : mémoire agentique dérivée, optionnelle et reconstructible ; cycle brut → candidat → validé ; Markdown/Git restent canoniques ; articulation avec Mnemosyne et FTS5 ; POC TencentDB uniquement isolé, local et après nouvel arbitrage. Statut : document de travail, aucune intégration ni installation appliquée (T-PLAN-7).
- `plans/2026-07-29-pro-workflow-integration.md` — plan d’amélioration issu de l’audit de Pro Workflow : garde-fou Git destructif, scan local de secrets, boucle de correction gouvernée, clôture déterministe Code, index FTS5 reconstructible pour Knowledge et inventaire auditable des hooks. Statut : document de travail, six améliorations à arbitrer séparément ; aucune intégration appliquée (T-PLAN-6).
