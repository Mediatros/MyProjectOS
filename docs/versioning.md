...
2. Écrire le nouveau numéro dans **trois** endroits, toujours ensemble : `VERSION`, le champ `version_methode` de `PROJECT.md`, et la ligne « Version courante : **X.Y.Z** » de `README.md` (le dépôt méthode n'est pas un projet installé : `--update-method` ne s'applique pas à lui-même). `publish-mirror.sh` réaligne le README à l'export, mais ne corrige pas la source : un README d'atelier laissé en arrière est un numéro faux affiché pendant tout le cycle de développement.
...
6. Commiter, poser le tag `git tag vX.Y.Z`, pousser sur l'atelier (`git push && git push origin vX.Y.Z`). Identité de contribution : celle du titulaire du dépôt, sans trailer (DEC-0049). **Puis attendre le verdict de la CI** : `gh run watch --exit-status`. Un push n'est pas une publication tant que le run n'est pas vert ; un commit `docs:` peut casser le lint (CHG-20260823-0840).
