...
Intégrer dans MyProjectOS une extension officielle `knowledge` inspirée du framework Grok et validée par l’organisation réelle du projet `Projet Zeta`.
...
Constat : il ne faut pas remplacer MyProjectOS par le framework Grok. Il faut en faire une extension officielle `knowledge`, avec `Projet Zeta` comme cas de dogfood réel.
...
3. Ne pas copier les contenus métier Projet Zeta dans MyProjectOS.
