...
Avant de créer un dossier à la racine (numéroté ou non) : vérifier dans `structures/*-tree.md` et sur le disque qu'aucune variante proche du nom canonique n'existe déjà (singulier/pluriel, casse, accents, abréviation). Un dossier racine mal nommé attire du contenu pendant des jours avant d'être détecté — voir un RETEX de l'atelier.

`02_sujets/` (Life/Hybrid uniquement, optionnel) redéfinit le slot `02` du Core pour l'organisation par sujets : voir `structures/life-tree.md` (DEC-0032). Ses sous-dossiers suivent le patron `Sxx_NomDuSujet` (deux chiffres, un sujet de fond par dossier, nom dérivé du titre par `sh scripts/sujet.sh new "Titre"` : accents retirés, espaces en underscores) ; Code garde `02_work/` sans changement. Chaque sous-dossier porte un `PROGRESS.md` à en-tête `sujet` / `titre` / `statut` / `derniere_maj` / `etat` / `prochaine_action` / `prochaine_echeance` ; statuts de sujet : `actif`, `en pause`, `clos` (DEC-0053). `02_sujets/INDEX.md` est la carte des sujets (objet de chacun, sans statut ni date). Le bloc « sujets » du `PROGRESS.md` racine, entre `<!-- sujets:debut -->` et `<!-- sujets:fin -->`, est généré par `scripts/sync-progress.sh` et ne s'édite pas à la main.
