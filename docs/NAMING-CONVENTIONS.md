...
Avant de créer un dossier à la racine (numéroté ou non) : vérifier dans `structures/*-tree.md` et sur le disque qu'aucune variante proche du nom canonique n'existe déjà (singulier/pluriel, casse, accents, abréviation). Un dossier racine mal nommé attire du contenu pendant des jours avant d'être détecté — voir le RETEX correspondant.

`02_sujets/` (Life/Hybrid uniquement, optionnel) redéfinit le slot `02` du Core pour l'organisation par sujets : voir `structures/life-tree.md` (DEC-0032). Ses sous-dossiers suivent le patron `Sxx_NomDuSujet` (deux chiffres, un sujet de fond par dossier) ; Code garde `02_work/` sans changement.
